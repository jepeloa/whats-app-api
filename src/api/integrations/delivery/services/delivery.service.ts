...
6. Confidence "high" = el comentario claramente nombra tambos/ubicaciones de destino. "medium" = probable pero no seguro. "low" = no se puede determinar.`,
  ...
  private buildSystemPrompt(delivery: DeliveryTracking & { locations: any[] }): string {
    ...
    const pendingPrincipal = pendingAll
      .filter(isPrincipal)
      .map((l) => `- ${l.nombre}: ${l.direccion}`)
      .join('\n');
    const pendingSecundaria = pendingAll
      .filter((l) => !isPrincipal(l))
      .map((l) => `- ${l.nombre}: ${l.direccion}`)
      .join('\n');
...
- Si el camionero envía algo inesperado (emojis, audio, foto), respondé amablemente y preguntá si necesita registrar algo.`;
  ...
  async create(instanceName: string, data: CreateDeliveryDto) {
    ...
    if (existingPesada) {
      ...
      if (['completed', 'partial', 'not_delivered'].includes(existingPesada.status)) {
        ...
        throw new Error(
          `La pesada #${data.idPesada} ya tiene un pedido activo en esta instancia. Cerrelo antes de recrearlo.`,
        );
      }
    }
  ...
  async activateNextQueued(instanceName: string, remoteJid: string, instanceId: string): Promise<void> {
    ...
    // Reconstruct a CreateDeliveryDto-shaped object to reuse buildInitialMessage
    // (which relies on metadata for principales/alternativas formatting).
    const dtoLike = {
      idPesada: next.idPesada,
      phoneNumber: next.remoteJid.replace('@s.whatsapp.net', ''),
      ...
      pesoUn: next.pesoUn,
      ubicaciones: next.locations.map((l) => ({ nombre: l.nombre, direccion: l.direccion, orden: l.orden })),
      metadata: (next.metadata as Record<string, any>) || {},
    } as CreateDeliveryDto;

    const initialMessage = this.buildInitialMessage(dtoLike);
    ...
    for (const action of aiResponse.actions) {
      if (action.action === 'query_pending') {
        if (alreadyListedRecently) {
          this.logger.info(`Skipping query_pending listing for pesada ${delivery.idPesada} — already listed recently`);
          continue;
        }
...
Respondé siempre en español, breve y amigable.`;
    ...
    if (pendingLocations.length === 0) {
      ...
      if (!forceComplete) {
        ...
        if (!pending) {
          ...
          this.logger.info(`All locations delivered for ${delivery.idPesada}; pre-close prompt deferred to next turn.`);
        }
        return;
      }
  ...
  private async saveToSigo(delivery: any): Promise<void> {
    ...
    try {
      ...
      if (!traslado) {
        this.logger.warn(
          `Could not find or create traslado in SIGO for pesada ${delivery.idPesada}, skipping write-back`,
        );
        return;
      }
      ...
      const ubicacionesConCarga = delivery.locations.filter(
        (l: any) => l.kilosDescargados && l.kilosDescargados > 0,
      ).length;
      ...
      if (diferencia > 0 || pesoNeto > 0) {
        ...
        this.logger.info(`Traslado ${traslado.tras_id}: diferencia=${diferencia} kg, tipo="${tipoDiferencia || ''}"`);
      }
